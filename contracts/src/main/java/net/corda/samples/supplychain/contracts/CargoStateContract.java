...
public class CargoStateContract implements Contract {
    ...
    public void verify(LedgerTransaction tx) {
        ...
        final CommandWithParties<CargoStateContract.Commands> command = requireSingleCommand(tx.getCommands(), CargoStateContract.Commands.class);

        List<ContractState> inputs = tx.getInputStates();
        List<ContractState> outputs = tx.getOutputStates();

        if (command.getValue() instanceof CargoStateContract.Commands.Create) {
            ...
            requireThat(require -> {
                ...
                CargoState output = (CargoState) outputs.get(0);
